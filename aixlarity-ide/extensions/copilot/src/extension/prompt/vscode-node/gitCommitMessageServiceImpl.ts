...
export class GitCommitMessageServiceImpl implements IGitCommitMessageService {
	...
	async generateCommitMessage(repository: Repository, cancellationToken: CancellationToken = CancellationToken.None): Promise<string | undefined> {
		...
		return window.withProgress({ location: ProgressLocation.SourceControl }, async () => {
			...
			// [GCD IDE] Override Copilot generation with currently loaded GCD AI Model
			const home = require('os').homedir();
			const gcdBin = require('path').join(home, 'PycharmProjects', 'ClaudeCode', 'target', 'debug', 'gcd');
			const { execFile } = require('child_process');
			const prompt = `Write a concise, conventional commit message for these staged changes in the repository '${repositoryName}'. Output ONLY the commit message and nothing else.\n\nChanges:\n${diffs.join('\n\n')}`;
			
			const commitMessage = await new Promise<string | undefined>((resolve) => {
				execFile(gcdBin, ['exec', '--json', prompt], { cwd: repository.rootUri.fsPath, maxBuffer: 1024 * 1024 * 10 }, (error: any, stdout: string) => {
					if (error) {
						console.error('[GCD SCM]', error);
						resolve(undefined);
						return;
					}
					try {
						const res = JSON.parse(stdout);
						resolve(res.result.final_response.trim());
					} catch (e) {
						console.error('[GCD SCM JSON Parse Error]', e, stdout);
						resolve(undefined);
					}
				});
			});
