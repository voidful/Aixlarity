...
import { app, Details, GPUFeatureStatus, powerMonitor, protocol, session, Session, systemPreferences, WebFrameMain, webContents } from 'electron';
...
export class CodeApplication extends Disposable {
	...
	async startup(): Promise<void> {
		...
		// [Aixlarity IDE] Automatic Aixlarity Daemon Lifecycle Management
		// The daemon is tightly bound to IDE lifecycle:
		//   - Spawned on IDE startup
		//   - Killed on IDE shutdown (any reason)
		//   - Auto-restarts on unexpected exit (up to 5 times)
		let aixProcess: ReturnType<typeof spawn> | undefined;
		let aixRestartCount = 0;
		const AIX_MAX_RESTARTS = 5;
		let aixlarityShuttingDown = false;
		const sendDaemonStatus = (params: Record<string, unknown>) => {
			const line = JSON.stringify({ method: 'daemon_status', params }) + '\n';
			for (const wc of webContents.getAllWebContents()) {
				if (!wc.isDestroyed()) {
					wc.send('vscode:aixlarity:daemonOut', line);
				}
			}
		};

		const spawnAixlarityDaemon = async () => {
			try {
				sendDaemonStatus({ status: 'starting', message: 'Starting Aixlarity daemon...' });
				...
				this.logService.info(`[Aixlarity] Resolved binary path: ${aixBin}`);
				aixProcess = spawn(aixBin, ['serve'], {
					stdio: ['pipe', 'pipe', 'pipe'],
					detached: false
				});
				this.logService.info(`[Aixlarity] Spawning Aixlarity Daemon JSON-RPC... PID: ${aixProcess.pid}`);
				sendDaemonStatus({ status: 'ready', message: 'Aixlarity daemon started.', pid: aixProcess.pid });

				const stdoutDecoder = new StringDecoder('utf8');
				aixProcess.stdout!.on('data', (data: Buffer) => {
					try {
						const strData = stdoutDecoder.write(data);
						try { process.stdout.write(`[Aixlarity Daemon OUT] ${strData}`); } catch (_) {}
						for (const wc of webContents.getAllWebContents()) {
							if (!wc.isDestroyed()) {
								wc.send('vscode:aixlarity:daemonOut', strData);
							}
						}
					} catch (e) {
						console.error('[Aixlarity IPC] Failed to route stdout:', e);
					}
				});
				const stderrDecoder = new StringDecoder('utf8');
				aixProcess.stderr!.on('data', (data: Buffer) => {
					this.logService.error('[Aixlarity RPC ERR]', stderrDecoder.write(data));
				});
				aixProcess.on('error', error => {
					const message = `Aixlarity daemon failed to start: ${error.message}`;
					this.logService.error('[Aixlarity] Daemon process error:', error);
					sendDaemonStatus({ status: 'error', message });
				});
				aixProcess.on('exit', (code) => {
					this.logService.warn(`[Aixlarity] Daemon exited with code ${code}.`);
					aixProcess = undefined;
					// Auto-restart if not shutting down and under restart limit
					const willRestart = !aixlarityShuttingDown && aixRestartCount < AIX_MAX_RESTARTS;
					sendDaemonStatus({
						status: 'exited',
						message: willRestart ? 'Aixlarity daemon exited; restarting...' : 'Aixlarity daemon exited.',
						code,
						willRestart
					});
					if (willRestart) {
						aixRestartCount++;
						this.logService.info(`[Aixlarity] Auto-restarting daemon (attempt ${aixRestartCount}/${AIX_MAX_RESTARTS})...`);
						setTimeout(() => spawnAixlarityDaemon(), 1000);
					}
				});

				// Attach handles to global context so the Extension Host
				// and Workbench services can retrieve them later.
				(globalThis as any).__aixlarity_daemon_stdin = aixProcess.stdin;
				(globalThis as any).__aixlarity_daemon_stdout = aixProcess.stdout;
			} catch (e) {
				this.logService.error('[Aixlarity] Failed to start Aixlarity Daemon:', e);
				sendDaemonStatus({ status: 'error', message: e instanceof Error ? e.message : 'Failed to start Aixlarity daemon.' });
			}
		};

		// Spawn on startup
		await spawnAixlarityDaemon();

			// Handle incoming IPC messages from the renderer (Sidebar chat)
			validatedIpcMain.on('vscode:aixlarity:daemonIn', (event, text: string) => {
				if (aixProcess?.stdin && !aixProcess.stdin.destroyed) {
					aixProcess.stdin.write(text.endsWith('\n') ? text : text + '\n');
				} else {
					const line = JSON.stringify({
						method: 'daemon_status',
						params: {
							status: 'error',
							message: 'Aixlarity daemon is not running.'
						}
					}) + '\n';
					event.sender.send('vscode:aixlarity:daemonOut', line);
				}
			});

		// Kill daemon on IDE shutdown (bound to ALL shutdown reasons)
		Event.once(this.lifecycleMainService.onWillShutdown)(e => {
			aixlarityShuttingDown = true;
			if (aixProcess) {
				this.logService.info(`[Aixlarity] IDE shutting down (reason: ${e.reason}), killing daemon PID: ${aixProcess.pid}`);
				aixProcess.kill('SIGTERM');
				// Force kill after 3s if still alive
				setTimeout(() => {
					if (aixProcess) {
						this.logService.warn('[Aixlarity] Daemon did not exit in time, force killing...');
						aixProcess.kill('SIGKILL');
					}
				}, 3000);
			}
