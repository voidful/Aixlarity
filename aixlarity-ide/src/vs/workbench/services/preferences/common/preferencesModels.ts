...
export class DefaultSettings extends Disposable {
	...
	private compareGroups(c1: ISettingsGroup, c2: ISettingsGroup): number {
		const c1HasOrder = typeof c1?.order === 'number';
		const c2HasOrder = typeof c2?.order === 'number';

		if (c1HasOrder && c2HasOrder && c1.order !== c2.order) {
			return c1.order! - c2.order!;
		}

		if (c1HasOrder !== c2HasOrder) {
			return c1HasOrder ? -1 : 1;
		}
