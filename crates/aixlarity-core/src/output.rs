...
impl From<&ProviderProfile> for ProviderJson {
    fn from(p: &ProviderProfile) -> Self {
        Self {
            ...
            source_kind: p.source.kind().to_string(),
            scope: p.source.scope().to_string(),
...
#[derive(Serialize)]
pub(crate) struct ProviderModelsJson {
    pub id: String,
...
#[derive(Serialize)]
pub(crate) struct CatalogListJson {
    pub workspace: String,
    pub trust: String,
    pub commands: Vec<CatalogCommandJson>,
    pub skills: Vec<CatalogSkillJson>,
}

#[derive(Serialize)]
pub(crate) struct CatalogCommandJson {
    pub name: String,
    pub description: String,
    pub source_path: String,
}

impl From<&CustomCommand> for CatalogCommandJson {
    fn from(command: &CustomCommand) -> Self {
        Self {
            name: command.name.clone(),
            description: command.description.clone(),
            source_path: display_path(&command.source_path),
        }
    }
}

#[derive(Serialize)]
pub(crate) struct CatalogSkillJson {
    pub name: String,
    pub summary: String,
    pub source_path: String,
}

impl From<&SkillDefinition> for CatalogSkillJson {
    fn from(skill: &SkillDefinition) -> Self {
        Self {
            name: skill.name.clone(),
            summary: skill.summary.clone(),
            source_path: display_path(&skill.source_path),
        }
    }
}
...
pub(crate) struct ExecJson {
    ...
    /// Truncated preview of the assembled prompt — the full prompt is NOT sent
    /// to the IDE to avoid multi-hundred-KB payloads per execution.
    pub prompt: String,
...
pub(crate) struct ExecResultJson {
    ...
    /// Lightweight summaries only — full arguments/results are stripped to
    /// prevent multi-MB payloads from accumulating in IDE renderer memory.
    pub tool_invocations: Vec<ToolInvocationJson>,
    pub token_usage: TokenUsageJson,
    // NOTE: events are NOT duplicated here — they already exist at ExecJson.events.
    // The previous duplication was a major memory leak source.
}
...
pub(crate) struct ToolInvocationJson {
    ...
    /// Compact summary of tool arguments — full values stripped to save memory.
    pub arguments_summary: String,
    /// Compact summary of tool result — full values stripped to save memory.
    pub result_summary: String,
}

impl From<&ToolInvocationRecord> for ToolInvocationJson {
    fn from(value: &ToolInvocationRecord) -> Self {
        let args_str = serde_json::to_string(&value.arguments).unwrap_or_default();
        let result_str = serde_json::to_string(&value.result).unwrap_or_default();
        Self {
            turn: value.turn,
            tool_name: value.tool_name.clone(),
            arguments_summary: truncate_text(&args_str, 200),
            result_summary: truncate_text(&result_str, 200),
        }
    }
}
...
pub(crate) fn render_exec_output(
    ...
    let json_data = ExecJson {
        ...
        // Send only a truncated preview of the prompt to prevent 400-500KB
        // payloads from accumulating in the IDE's V8 heap.
        prompt: truncate_text(&assembly.final_prompt, 500),
        ...
        events: execution_events
            ...
            .map(|e| match e {
                AgentEvent::ExecutionPrepared {
                    ...
                    pending_shell_command_count: *pending_shell_command_count,
                ...
                AgentEvent::ToolCallRequested {
                    ...
                    let args_str =
                        truncate_text(&serde_json::to_string(arguments).unwrap_or_default(), 500);
                    AgentEvent::ToolCallRequested {
                        turn: *turn,
                        call_id: call_id.clone(),
                        tool_name: tool_name.clone(),
                        arguments: serde_json::Value::String(args_str),
                    }
                }
                AgentEvent::ToolCallCompleted {
                    ...
                    let res_str =
                        truncate_text(&serde_json::to_string(result).unwrap_or_default(), 500);
                    AgentEvent::ToolCallCompleted {
                        turn: *turn,
                        call_id: call_id.clone(),
                        tool_name: tool_name.clone(),
                        result: serde_json::Value::String(res_str),
                        attachments: None,
                    }
                }
                other => other.clone(),
...
/// Inserts `section` before the `# Task` marker in a prompt string.
/// Currently unused after the Antigravity Request Mapper replaced legacy
/// text injection, but retained for potential future prompt manipulation.
#[allow(dead_code)]
