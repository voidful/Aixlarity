// Aixlarity — Hook System
//
// Lifecycle hooks that run shell commands before/after tool execution.
// Users configure hooks in `.aixlarity/hooks.json` (workspace) or `~/.aixlarity/hooks.json` (global).
//
...
impl HooksConfig {
    /// Load hooks from global config dir and workspace `.aixlarity/hooks.json`.
    ...
    pub fn load(global_config_dir: &Path, workspace_root: &Path) -> Self {
        ...
        // Workspace hooks
        let workspace_path = workspace_root.join(".aixlarity").join("hooks.json");
...
fn hook_env_vars(
    ...
    vars.insert("AIXLARITY_TOOL_NAME".to_string(), tool_name.to_string());
    vars.insert(
        "AIXLARITY_TOOL_ARGS".to_string(),
        serde_json::to_string(arguments).unwrap_or_default(),
    );
    if let Some(result) = result {
        vars.insert(
            "AIXLARITY_TOOL_RESULT".to_string(),
...
mod tests {
    ...
    fn hook_env_vars_includes_tool_info() {
        ...
        assert_eq!(vars["AIXLARITY_TOOL_NAME"], "read_file");
        assert!(vars["AIXLARITY_TOOL_ARGS"].contains("foo.rs"));
        assert!(vars["AIXLARITY_TOOL_RESULT"].contains("hello"));
    }
