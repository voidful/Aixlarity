// Aixlarity — Plugin System
//
// Dynamic tool loading from JSON definitions.
// Users place plugin JSON files in .aixlarity/plugins/ to extend agent capabilities.
    ...
    pub fn load(paths: &AppPaths, workspace: &Workspace, trust: &TrustState) -> io::Result<Self> {
        ...
        if !trust.restricts_project_config() {
            load_plugins_from_dir(
                &workspace.root.join(".aixlarity").join("plugins"),
                &mut plugins,
            )?;
        }
...
fn truncate_bytes(raw: &[u8], limit: usize) -> String {
    ...
    if rendered.len() <= limit {
        ...
        // Safe char-boundary truncation to avoid panicking on multi-byte UTF-8
