...
//
// Both files live in .aixlarity/ within the workspace. They are injected into the
...
impl MemoryStore {
    fn new(workspace_root: &Path, filename: &str) -> Self {
        let workspace =
        ...
        Self {
            path: workspace.local_data_dir().join(filename),
        }
    }
