...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> anyhow::Result<Value> {
        ...
        // Path-level ACL enforcement for multi-agent isolation.
        // When a sub-agent has allowed_paths or forbidden_paths set by the
        // coordinator, this check prevents writes outside the assigned scope.
        ctx.check_write_acl(&path)?;
        ...
        let tx_id = history
            .snapshot_before("write_file", &path)
            .unwrap_or_else(|_| "tx_failed".to_string());
            ...
            .unwrap_or(false)
        {
            ...
            res = super::embed_tool_events(
                res,
                vec![crate::agent::AgentEvent::ArtifactUpdated {
                    path: path.display().to_string(),
                    artifact_type,
                    summary,
                }],
