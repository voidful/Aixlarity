...
// Stored in `.aixlarity/cache/` with automatic TTL-based expiration.
...
pub fn build_cache_key(
    ...
    format!("aixlarity-cache-{:016x}", hash)
}
...
pub fn workspace_cache_dir(workspace_root: &Path) -> PathBuf {
    ...
    workspace.local_data_dir().join("cache")
}
...
mod tests {
    ...
    fn test_cache_dir() -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("aixlarity-cache-test-{}", unique))
    }
