...
async fn call_provider_api_inner(
    ...
    match options.provider.protocol {
        ...
        ProviderProtocol::ExternalCliStream => {
            external_cli::call_external_cli(options, messages).await
        }
    }
}
    ...
    match tool {
        Some(tool) => match tool.execute(call.arguments.clone(), ctx).await {
            Ok(mut result) => {
                ...
                let emitted_attachments = tool.take_output_attachments(&mut result);
                ToolExecutionOutcome {
                    result,
                    emitted_events,
                    emitted_attachments,
...
pub(super) struct ToolExecutionOutcome {
    ...
    pub emitted_attachments: Vec<crate::agent::AgentAttachment>,
}
