...
#[derive(Clone)]
pub struct StreamCallback(pub std::sync::Arc<dyn Fn(String) + Send + Sync>);

impl std::fmt::Debug for StreamCallback {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "StreamCallback")
    }
}

#[derive(Clone)]
pub struct EventCallback(pub std::sync::Arc<dyn Fn(AgentEvent) + Send + Sync>);
...
pub struct AgentRunOptions {
    ...
    pub initial_attachments: Option<Vec<AgentAttachment>>,
    pub max_turns: usize,
    pub api_key: String,
    ...
    /// Optional handler for streaming output tokens (useful for JSON-RPC / UI frontends)
    pub stream_handler: Option<StreamCallback>,
    /// Optional handler for streaming live events (Tool calls, Turn events)
    pub event_handler: Option<EventCallback>,
    /// Source session history for Antigravity request mapping
    pub source_session: Option<crate::session::SessionRecord>,
}
...
pub enum AgentEvent {
    ...
    // --- Merge Gate events (multi-agent isolation) ---
    // Design: These events provide an auditable trace of the merge gate's
    // decisions, enabling the IDE to show per-agent patch status and
    // enabling post-hoc debugging of multi-agent coordination issues.
    MergeGateStarted {
        depth: usize,
        patch_count: usize,
    },
