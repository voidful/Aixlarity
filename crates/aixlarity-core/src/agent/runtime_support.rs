...
pub(super) fn compact_messages(messages: &mut Vec<AgentMessage>) {
    ...
    let keep_last = 4;
    let mut split_index = messages.len().saturating_sub(keep_last);

    // Prevent orphan Tool messages by walking backward to the Assistant message
    while split_index > 1 {
        if messages[split_index].role == "tool" {
            split_index -= 1;
        } else if messages[split_index].role == "assistant"
            && messages[split_index].tool_calls.is_some()
        {
            // Include the assistant message that spawned the tool calls
            break;
        } else {
            // It's a safe boundary to split
            break;
        }
    }
    ...
    let middle_count = split_index - 1;
    ...
    for message in &messages[1..=middle_count] {
        match message.role.as_str() {
            ...
            "user" => {
                let truncated: String = message.content.chars().take(60).collect();
    ...
    let tail = messages.split_off(split_index);
    messages.clear();
    messages.push(first);
...
pub(super) fn git_auto_commit_safe(workspace: &Path, summary: &str) {
    ...
    // Truncate at a safe char boundary — `summary[..72]` would panic on multi-byte
    // UTF-8 (e.g. Chinese, emoji). Use char_indices to find the last valid boundary.
    ...
    let commit_msg = if has_public_remote(workspace) {
        ...
        format!("aixlarity: {}", sanitized)
    } else {
        format!("aixlarity: {}", truncated)
    };
