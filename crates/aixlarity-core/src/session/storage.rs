...
use super::model::{
    CheckpointRecord, SessionExecutionData, SessionListEntry, SessionRecord, SessionTurnRecord,
};
...
pub fn save_checkpoint(base_dir: &Path, assembly: &PromptAssembly) -> io::Result<PathBuf> {
    ...
    let slug = slugify(
        assembly
            .active_command
            .as_deref()
            .or(assembly.active_skill.as_deref())
            .unwrap_or("session"),
    );
    let file_name = format!("{}-{}.md", stamp, slug);
    let path = base_dir.join(file_name);
    let body = format!(
        "# Aixlarity Checkpoint\n\n- Workspace: {}\n- Provider: {}\n- Trust: {}\n- Sandbox: {}\n\n## Prompt\n\n{}",
...
pub fn save_new_session(
    ...
    mode: &str,
    execution: Option<&SessionExecutionData>,
    ...
    let turn = build_turn(1, user_input, assembly, now, mode, execution);
...
pub fn append_session_turn(
    ...
    record.turns.push(build_turn(
        next_index, user_input, assembly, now, mode, execution,
    ));
...
pub fn list_sessions(base_dir: &Path) -> io::Result<Vec<SessionListEntry>> {
    ...
    for entry in fs::read_dir(base_dir)? {
        let entry = entry?;
        let path = entry.path();
        if !path.is_dir() || !path.join("session.txt").exists() {
            continue;
        }
...
pub(super) fn summarize_task(input: &str) -> String {
    let line = input
        .lines()
...
fn write_turn_file(path: &Path, turn: &SessionTurnRecord) -> io::Result<()> {
    ...
    header.push(format!("mode={}", turn.mode));
    ...
    header.push(format!("input_bytes={}", turn.input.len()));
    header.push(format!("prompt_bytes={}", turn.prompt.len()));
    header.push(format!(
        "response_bytes={}",
        turn.final_response.as_deref().unwrap_or("").len()
    ));
    let events_json = if turn.events.is_empty() {
        String::new()
    } else {
        serde_json::to_string(&turn.events).map_err(|error| {
            invalid_data(format!("failed to serialize session events: {}", error))
        })?
    };
    header.push(format!("events_json_bytes={}", events_json.len()));
    header.push(format!("turns_used={}", turn.turns_used));
    header.push(format!(
        "tool_invocation_count={}",
        turn.tool_invocation_count
    ));
    header.push(format!("prompt_tokens={}", turn.prompt_tokens));
    header.push(format!("completion_tokens={}", turn.completion_tokens));
    header.push(format!("total_tokens={}", turn.total_tokens));
    header.push(format!("api_calls={}", turn.api_calls));
...
fn read_turn_file(path: &Path) -> io::Result<SessionTurnRecord> {
    ...
    let response_bytes = parse_optional_usize_field(&fields, "response_bytes").unwrap_or(0);
    let events_json_bytes = parse_optional_usize_field(&fields, "events_json_bytes").unwrap_or(0);
    let payload_bytes = payload.as_bytes();
    if payload_bytes.len() < input_bytes + prompt_bytes + response_bytes + events_json_bytes {
    ...
    let final_response = if response_bytes == 0 {
        None
    } else {
        Some(
            String::from_utf8(
                payload_bytes
                    [input_bytes + prompt_bytes..input_bytes + prompt_bytes + response_bytes]
                    .to_vec(),
            )
            .map_err(|_| invalid_data("session response is not valid utf-8"))?,
        )
    };
    let events = if events_json_bytes == 0 {
        Vec::new()
    } else {
        let start = input_bytes + prompt_bytes + response_bytes;
        let raw = String::from_utf8(payload_bytes[start..start + events_json_bytes].to_vec())
            .map_err(|_| invalid_data("session events payload is not valid utf-8"))?;
        serde_json::from_str(&raw)
            .map_err(|error| invalid_data(format!("invalid session events payload: {}", error)))?
    };

    Ok(SessionTurnRecord {
        index: parse_usize_field(&fields, "index")?,
        timestamp_secs: parse_u64_field(&fields, "timestamp_secs")?,
        mode: optional_field(&fields, "mode").unwrap_or_else(|| "plan".to_string()),
        ...
        prompt,
        final_response,
        turns_used: parse_optional_usize_field(&fields, "turns_used").unwrap_or(0),
        tool_invocation_count: parse_optional_usize_field(&fields, "tool_invocation_count")
            .unwrap_or(0),
        prompt_tokens: parse_optional_usize_field(&fields, "prompt_tokens").unwrap_or(0),
        completion_tokens: parse_optional_usize_field(&fields, "completion_tokens").unwrap_or(0),
        total_tokens: parse_optional_usize_field(&fields, "total_tokens").unwrap_or(0),
        api_calls: parse_optional_usize_field(&fields, "api_calls").unwrap_or(0),
        events,
    })
}
...
fn build_turn(
    ...
    let execution = execution.cloned().unwrap_or_default();
    SessionTurnRecord {
        index,
        timestamp_secs,
        mode: mode.to_string(),
        ...
        tool_invocation_count: execution.tool_invocation_count,
...
fn parse_optional_usize_field(values: &BTreeMap<String, String>, key: &str) -> Option<usize> {
...
fn slugify(value: &str) -> String {
    ...
    for ch in value.chars().take(64) {
        ...
        } else if (ch == '-' || ch == '_' || ch == ':' || ch == ' ') && !output.ends_with('-') {
            output.push('-');
        }
    }
    if output.is_empty() {
...
pub fn delete_session(base_dir: &Path, session_id: &str) -> io::Result<()> {
    let dir = session_dir(base_dir, session_id);
    if dir.exists() {
