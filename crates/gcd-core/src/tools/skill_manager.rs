// GemiClawDex — Skill Manager Tool
//
...
//
// Skills are stored in ~/.gcd/skills/{name}/SKILL.md.
...
fn validate_skill_name(name: &str) -> Result<(), String> {
    ...
    if !name.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
        return Err("Skill name may only contain alphanumeric characters, hyphens, and underscores.".into());
    }
    Ok(())
}
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> anyhow::Result<Value> {
        ...
        match action {
            ...
            "edit" => {
                ...
                // Only allow editing user-created skills (under ~/.gcd/skills/)
                if !is_user_skill(&skill_dir) {
                    return Ok(serde_json::json!({
                        "error": "Cannot edit bundled or project-level skills. Only user-created skills in ~/.gcd/skills/ can be modified."
                    }));
                }
