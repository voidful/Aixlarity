...
pub fn run_in_container(
    ...
    let image = std::env::var("GCD_CONTAINER_IMAGE")
        .ok()
        .filter(|v| !v.is_empty());
    let timeout = std::env::var("GCD_CONTAINER_TIMEOUT")
        .ok()
        .and_then(|v| v.parse::<u64>().ok());
    let network = std::env::var("GCD_CONTAINER_NETWORK")
        .ok()
        .map(|v| v == "1" || v == "true")
        .unwrap_or(false);
