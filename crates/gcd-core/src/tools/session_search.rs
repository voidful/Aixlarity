// GemiClawDex — Session Search Tool
//
// Searches past session transcripts stored as JSONL files in ~/.gcd/sessions/.
...
//
// Flow:
//   1. Scan session JSONL files in ~/.gcd/sessions/
...
impl Tool for SessionSearchTool {
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> anyhow::Result<Value> {
        ...
        let sessions_dir = AppPaths::detect(std::env::current_dir().unwrap_or_default())
            .sessions_dir();
        ...
        for session_path in session_files {
            ...
            for line in content.lines() {
                ...
                if keywords.iter().all(|kw| lower_content.contains(kw.as_str())) {
